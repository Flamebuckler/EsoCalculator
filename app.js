document.addEventListener('DOMContentLoaded', () => {
	...
	function createRow({
		...
		const initialCount = maxAttr
			? Math.min(initialCountRaw, parseIntSafe(maxAttr))
			: initialCountRaw;
		if (countInp && String(initialCount) !== String(countInp.value))
			countInp.value = String(initialCount);
		if (tdValue) {
	...
	// Summary positioning used to toggle an "in-main" class when the
	// page was scrolled to the bottom.  That behaviour has been removed;
	// #summary now stays fixed to the viewport bottom at all times, so the
	// extra logic is unnecessary.
	//
	// (If you ever need to re-enable the old behaviour, see earlier
